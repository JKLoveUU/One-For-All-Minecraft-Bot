...
const schematic = { //統一轉成litematic那樣
    ...
    loadFromLitematic: async function (path) {
        ...
        // console.log(tglt.palette.length,(tglt.palette.length).toString(2),(tglt.palette.length).toString(2).length)
        let bittemp = Math.max(tglt.palette.length-1,defaultBits)
        const bitsPerEntry = BigInt(bittemp).toString(2).length;
...
class sch {
    ...
    palette = [{ Name: air }];
    /**
     * if blockState not in add it 這裡可能有點問題
     * { Name: "blockname",pr... }
     */
    getPaletteIndex(blockState){
        let index = this.palette.findIndex(item => {
            return item.Name === blockState.Name && JSON.stringify(item.p) === JSON.stringify(blockState.p);
        }); 
        if(index == -1){
    ...
    changeMaterial(from, to,pro){
        ...
        let oldProps = this.palette[idx]?.Properties
        if(oldProps && !pro) {
            res.Properties = oldProps
        }
