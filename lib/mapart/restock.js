...
async function mp_material(task) {
    ...
    let boxMode
    let bottomMode
    mapartState.Materialsdata['stationServer'] = task.content[2]
    mapartState.Materialsdata['stationWarp'] = task.content[3]
    ...
    if (task.content[4] - task.content[7] === 0) {
        ...
        for (z; z >= +task.content[9]; z--) {
            // 先飛到該 xz 欄正上方,確保 chunk 載入後 getBlock 才讀得到正確方塊
            // (站台很長時,遠端欄位若沒載入會被讀成 air 而誤判材料)。
            await pathfinder.astarfly(bot, new Vec3(x, y, z), null, null, null, true)
            for (let i = y - 5; i >= -64; i--) {
                let block = await bot.world.getBlock(new Vec3(x, i, z))
                if (block.name === ('air' || 'cave_air' || 'bedrock')) {
                    break
                }
                let materialsPush = [block.name, [x, y, z, boxMode, bottomMode]]
                console.log(materialsPush)
                mapartState.Materialsdata.materials.push(materialsPush);
            }
        }
    }
...
async function mp_file(task) {
    ...
    fs.writeFileSync(filePath, JSON.stringify(mapartState.Materialsdata, null, 2));
    await taskreply(task,
...
module.exports = { stationRestock, mp_material, mp_file }
