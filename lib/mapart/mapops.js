...
const { getItemFrame, getMapItemByMapIDInInventory, pickMapItem, inv_sort, moveToEmptySlot, getEmptySlot, getMapIdFromItem, isMapItemNamed } = require('./utils')
    ...
    for (let dx = 0; dx < mapart_open_cfg_cache["open"]["width"]; dx++) {
        for (let dy = 0; dy < mapart_open_cfg_cache["open"]["height"]; dy++) {
            ...
            if (currentIF?.metadata && currentIF?.metadata[8]?.itemId == mcData.itemsByName['filled_map'].id) {
                csmp.mapid = getMapIdFromItem(currentIF.metadata[8]);
                csmp.finish = true;
            }
            mpstate.push(csmp)
        }
    ...
    async function openMap(mps) {
        ...
        while (try_open_c < 10) {
            ...
            if (bot.inventory?.slots[44]?.name == "filled_map") {
                mpID = getMapIdFromItem(bot.inventory.slots[44]);
                console.log(mpID)
                break;
            }
            try_open_c++
    ...
    async function putMapON() {
        ...
        for (let inv_i = 9; inv_i < 44; inv_i++) {
            if (bot.inventory.slots[inv_i] && bot.inventory.slots[inv_i].name == 'filled_map') {
                let lv = false;
                let mpID = getMapIdFromItem(bot.inventory.slots[inv_i]);
                ...
                if (mps) {
                    ...
                    while (fail_c < 10) {
                        ...
                        if (check && getMapIdFromItem(check.metadata[8]) == mpID) {
                            break;
                        }
                        fail_c++;
                    }
...
async function mp_name(task) {
    ...
    for (let i = 0; i < mapart_name_cfg_cache["wrap"]["width"] * mapart_name_cfg_cache["wrap"]["height"]; i++) {
        ...
        if (currentIF && currentIF?.metadata && currentIF?.metadata[8]?.itemId == mcData.itemsByName['filled_map'].id) {
            mps.hasmap = true
            mps.mapid = getMapIdFromItem(currentIF.metadata[8]);
        }
        if (mps.hasmap) {
            mps.named = isMapItemNamed(currentIF.metadata[8]);
        }
        maparts.push(mps)
    }
    for (let i = 0; i < mapart_name_cfg_cache["wrap"]["width"] * mapart_name_cfg_cache["wrap"]["height"]; i++) {
        ...
        while (fail_c < 10) {
            ...
            await sleep(1000)
            let check = getItemFrame(mps.pos);
            if (check && getMapIdFromItem(check.metadata[8]) == mps.mapid) {
                break;
            }
            fail_c++;
        }
...
async function mp_copy(task) {
    ...
    for (let i = 0; i < box_amount; i++) {
        ...
        for (let shu_index = 0; shu_index < 27 && checkIndex < maparts.length; shu_index++, checkIndex++) {
            ...
            else if (getMapIdFromItem(shulker_box.slots[shu_index]) != maparts[checkIndex].mapid) {
                console.log(shulker_box.slots[shu_index])
                console.log(getMapIdFromItem(shulker_box.slots[shu_index]), maparts[checkIndex].mapid, checkIndex)
    ...
    for (let i = 0; i < mapart_name_cfg_cache["wrap"]["width"] * mapart_name_cfg_cache["wrap"]["height"]; i++) {
        ...
        if (mapart_name_cfg_cache["wrap"].copy_amount - mps.amount == 64) {
            ...
            for (let ff = 27; ff <= 62; ff++) {
                if (getMapIdFromItem(shulker_box.slots[ff]) == mps.mapid && shulker_box.slots[ff]?.count == 64) {
                    tgmp = ff;
                    break;
                }
            }
            ...
            let tgmp = -1;
            for (let ff = 27; ff <= 62; ff++) {
                if (getMapIdFromItem(shulker_box.slots[ff]) == mps.mapid) {
                    tgmp = ff;
                    break;
                }
            }
...
async function mapCopy(cartography_table_pos, mapid, amount = 1) {
    ...
    async function mapCopyOne(cartography_table, mapid) {
        let targetMap = cartography_table.slots.find(item => item && getMapIdFromItem(item) == mapid)
