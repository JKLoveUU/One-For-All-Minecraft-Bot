...
async function mp_build(task) {
    ...
    const buildResult = await litematicPrinter.build_file(task, bot, litematicPrinter.model_mapart, cfg)
    if (buildResult === 'stopped') {
        bot.logger(true, 'INFO', process.argv[2], 'mapart build stopped; skip webhook and autonext');
        return;
    }
...
function sendBuildWebhook(bot, webhookClient, cfg, result, useTime) {
    ...
    webhookClient.send(wh).catch(e => console.warn("[webhook]", e.message))
}
