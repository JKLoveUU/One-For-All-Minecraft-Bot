...
const fs = require('fs');
const fsp = raequire('fs').promises
    ...
    build_file: async function (task, bot, model, cfg) {
        ...
        switch (model) {
            ...
            case this.model_building:
                await model_building_build(task, bot, cfg);
                break;
            case this.model_redstone:
                await model_redstone_build(task, bot, cfg);
                break;
            default:
 ...
 * @param {*} cfg 
 */
async function model_building_build(task, bot, cfg) {
    Item = require('prismarine-item')(bot.version)
    mcData = require('minecraft-data')(bot.version)
    console.log(cfg)
   ...
   // console.log(targetSch.Metadata.EnclosingSize)
    ...
    buildLoop: while (build_cache.currentLayer < build_cache.totalLayer) {// targetSch.palette.length
        ...
        let selectBlockAbsolutePos = build_cache.placement_origin.plus(selectBlockRelativePos)
        let selectBlockBotStandPos = selectBlockAbsolutePos.offset(0, 2, 0)
        //console.log(blockInCD.indexOf(selectBlockIndex) == -1, bot.blockAt(selectBlockAbsolutePos)?.name == currentPalette.Name)
        if (blockInCD.indexOf(selectBlockIndex) == -1 && checkBlock(bot.blockAt(selectBlockAbsolutePos),currentPalette)==1){//bot.blockAt(selectBlockAbsolutePos)?.name == currentPalette.Name) {
            //console.log(currentPalette.Name, bot.blockAt(selectBlockAbsolutePos)?.name)
            currentPaletteBlocksState[selectBlockIndex] = 1;    //[2 狀態錯誤]
        } else {
            ...
            if (currentPaletteBlocksState[selectBlockIndex] == 0 && blockInCD.indexOf(selectBlockIndex) == -1) {
                if (bot.blockAt(selectBlockAbsolutePos) == null) continue
                //重新檢查該位置
                let checkBlockID = checkBlock(bot.blockAt(selectBlockAbsolutePos),currentPalette)
               // console.log(checkBlockID)
                if (checkBlockID == 1) {
                    ...
                    //console.log("調整tick")
                    try{
                        await bot.activateBlock(bot.blockAt(selectBlockAbsolutePos));
                    }catch(e){

                    }
                    blockInCD.push(selectBlockIndex)
                    setTimeout(function () {
                        blockInCD.shift();
                    ...
                    //
                    await sleep(32)
                    await placeWithProperties(bot, currentPalette, selectBlockAbsolutePos)
                    blockInCD.push(selectBlockIndex)
                    setTimeout(function () {
                        blockInCD.shift();
                    }, building_check_cooldown)
