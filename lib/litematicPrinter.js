...
async function model_mapart_build(task, bot, cfg) {
    ...
    let debug_findNextTotalCounter = 0;
    let debug_restock_counter = 0;
    let debug_restock_usetime = 0; // ms
    ...
    mapartBuild: while (build_cache.currentPalette < targetSch.palette.length) {
        ...
        */
                    ...
                    if (currentPaletteBlocksState[dBlocksIndex] == 0 && Block_In_CD.indexOf(dBlocksIndex) == -1) {
                        ...
                        if (hold == null || hold.name != currentPaletteName) {
                            ...
                            if (findMaterialSlot == -1) { //背包也沒有 回去補
                                ...
                                needReStock.push({ name: currentPaletteName, count: quantityrestock })
                                if(true){
                                    ...
                                    for (let crtRSC = build_cache.currentPalette + 1; crtRSC < targetSch.palette.length; crtRSC++) {
                                        ...
                                        if (bot.inventory.countRange(bot.inventory.inventoryStart, bot.inventory.inventoryEnd, mcData.itemsByName[targetSch.palette[sch_palette_order[crtRSC]].Name].id, null) > 0) {
                                            //這裡需要更精準的補充 之後再加											
                                            continue;
                                        }
                                        ...
                                        needReStock.push({ name: targetSch.palette[sch_palette_order[crtRSC]].Name, count: crtCanRSCount })
                                ...
                                for (let jj = 0; jj < needReStock.length; jj++) {
                                    console.log(`預估補充 ${needReStock[jj].name}(${build_cache.currentPalette + jj}) ${needReStock[jj].count}`);
                                }
                               // throw new Error("test")
                                ...
                                console.log(`\x1b[33m切換物品欄\x1b[0m ${findMaterialSlot} <-> 手 (44)`)
                        ...
                        if (false) {
                            //console.log()
                            console.log(`\x1b[32m放置\x1b[0m ${currentPaletteName} (${build_cache.currentPalette}) 於 a${dAbsolutePos} r${dRelativePos} dBI:${dBlocksIndex} r_i:${currentPosPlace_d_Index} ${targetSch.getBlockByIndex(currentPosPlace_d_Index).Name}`);
                        }
        ...
        if (true) {
            ...
            debug_findNextTotalCounter +=(debug_endFNext - deubg_startFNext)
        }
    ...
    function updateVisited(oldBlock, newBlock) {
        ...
        if (currentPaletteName == newBlock.name && currentPaletteName == oldBlock.name) {
            if(currentPaletteBlocksState[qindex]==0) currentPaletteBlocksState[qindex] = 1;
    ...
    // #DEBug
    console.log(`FN總耗時: ${debug_findNextTotalCounter} ms`)
}
