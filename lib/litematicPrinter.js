                    ...
                    if (currentPaletteBlocksState[dBlocksIndex] == 0 && Block_In_CD.indexOf(dBlocksIndex) == -1) {
                        ...
                        if (hold == null || hold.name != currentPaletteName) {
                            ...
                            if (findMaterialSlot == -1) { //背包也沒有 回去補
                                ...
                                bot.logger(false, 'INFO', process.argv[2], "\n=============補充材料=================")
                                ...
                                if (cfg.materialsMode == 'station') {
                                    ...
                                    for (let i = 0; i < needReStock.length; i++) {
                                        let quantity = await wms.queryQuantity(wms.cfg, needReStock[i].name)
                                        if (quantity < needReStock[i].count) {
                                            bot.logger(true, "ERROR", process.argv[2], `倉庫存量不足 ${needReStock[i].name} 需要:${needReStock[i].count} 庫存:${quantity}`)
                                            // pause = true
                                            await wms.prepare(bot, needReStock[i].name, needReStock[i].count)
                                            await sleep(3000)
                                            continue
                                        }
                                        // 計算需要的背包空間
                                        let stackSize = mcData.itemsByName[needReStock[i].name]?.stackSize || 64;
                                        let slotsNeeded = Math.ceil(needReStock[i].count / stackSize);
                                        ...
                                        if (emptySlots < slotsNeeded) {
                                            // bot.logger(true, "ERROR", `背包空間不足 ${needReStock[i].name} 需要:${slotsNeeded}格 剩餘:${emptySlots}格`);
                                            // pause = true;
                                            // break;
                                            needReStock[i].count = emptySlots * stackSize
                                            if (needReStock[i].count <= 0) break;
                                        }
                                        let winfo = await wms.toWithdrawInfo(needReStock[i].name, needReStock[i].count, 'normal')
                                        let operations = await wms.getDeposit(wms.cfg, winfo)
                                        if (!operations) continue
                                        for (let j = 0; j < operations.length; j++) {
                                            let result = await wms.executeOperation(bot, operations[j])
                                            if (!result) {
                                                bot.logger(true, "ERROR", process.argv[2], `提取 ${needReStock[i].name} 失敗`)
        ...
        if (!wheatherGetLayerPalette) {
            ...
            for (let i = 0; i < targetSch.palette.length; i++) {
                if (layerPaletteCountByIndex[i] == 0) {
                    ...
                    console.log("跨版本 跳過" + targetSch.palette[i].Name)
                    delete layerPaletteCountByIndex[i]
                    delete layerPaletteBlocksByIndex[i]
                } else if (BLOCK_SKIP_LIST.includes(targetSch.palette[i].Name)) {
                    console.log("BLOCK_SKIP_LIST 跳過" + targetSch.palette[i].Name)
                    delete layerPaletteCountByIndex[i]
                    delete layerPaletteBlocksByIndex[i]
                } else if (targetSch.palette[i]?.Properties?.hanging == 'true') {
                    console.log("hanging 跳過" + targetSch.palette[i].Name)
                    delete layerPaletteCountByIndex[i]
                    delete layerPaletteBlocksByIndex[i]
                } else if (targetSch.palette[i]?.Properties?.face == 'ceiling'){
                    console.log("face ceiling 跳過" + targetSch.palette[i].Name)
                    delete layerPaletteCountByIndex[i]
                    delete layerPaletteBlocksByIndex[i]
                }
            }
        ...
        if (!props.facing) {
            ...
            switch (props.facing) {
                ...
                case 'east':
                    ...
                    direction = 5;
                    break;
                case 'north': cursorX = 0.5; cursorY = 0.5; cursorZ = 0.8; direction = 2; break;
                case 'south': cursorX = 0.5; cursorY = 0.5; cursorZ = 0.2; direction = 3; break;
                // case 'west': cursorX = 0.8; cursorY = 0.5; cursorZ = 0.5; direction = 4; break;   // 不知道為啥蓋成 south 
                // case 'east': cursorX = 0.2; cursorY = 0.5; cursorZ = 0.5; direction = 5; break;
            }
            pitch = 0
        }
    }
