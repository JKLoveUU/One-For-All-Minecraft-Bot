...
async function mp_copy(task) {
    ...
    //await pathfinder.astarfly(bot, new Vec3(mapart_name_cfg_cache.wrap.anvil_stand[0], mapart_name_cfg_cache.wrap.anvil_stand[1], mapart_name_cfg_cache.wrap.anvil_stand[2]))
    let mp_origin = new Vec3(mapart_name_cfg_cache.wrap.origin[0], mapart_name_cfg_cache.wrap.origin[1], mapart_name_cfg_cache.wrap.origin[2])
    let mp_shu_origin = new Vec3(mapart_name_cfg_cache.wrap.copy_f_box[0], mapart_name_cfg_cache.wrap.copy_f_box[1], mapart_name_cfg_cache.wrap.copy_f_box[2])
    let maparts = []
    let facing = mapart_name_cfg_cache["wrap"]["facing"]
    ...
    for (let i = 0; i < mapart_name_cfg_cache["wrap"]["width"] * mapart_name_cfg_cache["wrap"]["height"]; i++) {
        ...
        /*
        if (bot.inventory.slots[43]) {
            let eps = bot.inventory.firstEmptyInventorySlot(false)
            await bot.simpleClick.leftMouse(43)
            await bot.simpleClick.leftMouse(eps)
        }
        let test = getMapItemByMapIDInInventory(mps.mapid)
        let t2 = test.slot
        if (t2 != 44) {
            await bot.simpleClick.leftMouse(t2)
            await bot.simpleClick.leftMouse(44)
            await bot.simpleClick.leftMouse(t2)
        }
        */
        //await sleep(10)
        // for (let am = 0; am < mapart_name_cfg_cache["wrap"].copy_amount; am++) {
        //     let e_map = getMapItemByMapIDInInventory(mps.mapid)
        //     console.log(e_map)
        //     //console.log(mpPrepareToCopy)
        //     await bot.simpleClick.leftMouse(e_map.slot)
        //     await bot.simpleClick.rightMouse(1)
        //     if (e_map.count != 1) await bot.simpleClick.leftMouse(e_map.slot)
        //     if (!bot.inventory.slots[2] || bot.inventory.slots[2]?.name != 'map') {
        //         let emptyMap = bot.inventory.findInventoryItem('map', null, false)
        //         await bot.simpleClick.leftMouse(emptyMap.slot)
        //         await bot.simpleClick.rightMouse(2)
        //         await bot.simpleClick.leftMouse(emptyMap.slot)
        //         //console.log(bot.inventory)
        //         //await sleep(1000)
        //         //console.log(bot.inventory)
        //         //await waitUpdateCraftSlot();
        //         async function waitUpdateCraftSlot() {
        //             let fail = false;
        //             try {
        ...
        //                         fail = true;
        //                         rej()
        //                         bot.inventory.slots[2]
        //                     }, 3_000)
        //                     while (!fail) {
        //                         wait()
        //                         if (bot.inventory.slots[2]) {
        ...
        //         // await bot.simpleClick.leftMouse(eps)
        //         //if(eps)
        //         // let tg_merge = getMapItemByMapIDInInventory(mps.mapid)
        //         // if(!tg_merge||tg_merge.count>=64)
        //     }
        //     //const item = new Item(e_map.type, 2, e_map.metadata,e_map.nbt)
        //     // let item = JSON.parse(JSON.stringify(e_map))
        //     // console.log(item)
        //     // bot.inventory.updateSlot(0, item)
        //     // console.log(bot.inventory)
        //     // let eps  =bot.inventory.firstEmptyInventorySlot(false)
        ...
        //     //         },{
        //     //         location: eps,
        //     //         item: Item.toNotch(bot.inventory.slots[0])
        //     //         }
        //     //     ],
        //     //     cursorItem: Item.toNotch(bot.inventory.slots[0])
        //     //   })
        //     // try{await bot.simpleClick.leftMouse(0)}catch(e){}
        //     // try{await bot.simpleClick.leftMouse(43)}catch(e){}
        //     // try{await bot.simpleClick.leftMouse(eps)}catch(e){}
        //     // try{await bot.simpleClick.leftMouse(43)}catch(e){}
        //     // try{await bot.simpleClick.leftMouse(eps)}catch(e){}
        //     // try{await bot.simpleClick.leftMouse(43)}catch(e){}
        //     // try{await bot.simpleClick.leftMouse(44)}catch(e){}


        //     bot.inventory.updateSlot(1, null)
        //     bot.inventory.updateSlot(2, null)
        //     await sleep(50)
        //     console.log(am)
        // }
        //await sleep(1000)
        //await bot.simpleClick.leftMouse(-999)
        // bot.inventory.count(951)
