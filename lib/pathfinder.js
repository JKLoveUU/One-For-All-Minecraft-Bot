    ...
    astarV2: async function (bot, target, mute = true) {
        ...
        while (true) {
            ...
            if (visited.has(getHash(locNow))) {
                bot.logger(false, 'DEBUG', bot.username, `\x1b[31mA*已訪問過 ${locNow} 嘗試更新目標 y+20 避免卡死 LERC: ${pathfinder.loopErrorRepeatCount}\x1b[0m`);
                let newy = target.y + 20;
                pathfinder.loopErrorRepeatCount++;
                if(newy > 320) newy = 320;
                bot.off('death', deathFlagSet);
                bot.off('forcedMove', forcedMove);
                if(pathfinder.loopErrorRepeatCount > 2){
                    bot.logger(false, 'DEBUG', bot.username, `\x1b[31mA*  LERC: ${pathfinder.loopErrorRepeatCount} >2 return false\x1b[0m`);
                    return false;
                }
