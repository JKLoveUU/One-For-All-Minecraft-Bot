...
const pathfinder = {
    ...
    astarfly: async function (bot, target, border1, border2, lastFlyTime, mute) {  //直接使用方塊座標
        ...
        await this.astarV2(bot,target,true)
        return;
        deathFlag = false;
...
async function astarV2FindPath(bot,target,mute=true){
    ...
    while(Object.keys(OPEN).length > 0){
        ...
        let current, lowestF_cost = Number.MAX_SAFE_INTEGER, lowestH_cost = Number.MAX_SAFE_INTEGER;
        let lowest_id = -1;
        for (let findLowest_cost = 0; findLowest_cost < Object.keys(OPEN).length; findLowest_cost++) {
            if (OPEN[Object.keys(OPEN)[findLowest_cost]].f <= lowestF_cost) {
                if (OPEN[Object.keys(OPEN)[findLowest_cost]].h < lowestH_cost) {
                    current = OPEN[Object.keys(OPEN)[findLowest_cost]];
                    lowestH_cost = current.h;
                    lowestF_cost = current.f;
                    lowest_id = findLowest_cost;
                }
        ...
        if (Date.now() - astar_start_time > 10_000) {
            end = current;
            bot.logger(false,'DEBUG',bot.username,`\x1b[31mA*超時 pos: ${start} target: ${target}\x1b[0m wait 50 ms`);
            await sleep(50);
            break;
        }
        // remove current from OPEN
        delete OPEN[Object.keys(OPEN)[lowest_id]]
        // add current to CLOSE
        CLOSE[getHash(current)] = current
        ...
        for (let i = 0; i < offset.length; i++) {
            for (let d =1; d<= maxExplore; d++){
                ...
                if(CLOSE[getHash(nowtest_down)]){
                    continue;
                }
                if (legitimate_block.indexOf(bot.blockAt(nowtest_down).name) >= 0 && legitimate_block.indexOf(bot.blockAt(nowtest_up).name) >= 0) {
                    // 排除牆壁 等 會擋路的
                    let nowTest_dd = new Vec3(current.x + offset[i][0] * d, current.y + offset[i][1] * d - 1, current.z + offset[i][2] * d);
                    if(bot.blockAt(nowTest_dd) != null){
                        let ndd_bname = bot.blockAt(nowTest_dd).name
                        if(ndd_bname.includes("wall")||ndd_bname.includes("fence")) continue
                    } else {
                    ...
                    OPEN[getHash(nowtest_down)] = nowtest_down;
                }else{
                    if(Math.abs(offset[i][1]) > 0){
                        continue
                    }else{
