...
// POST /api/v1/wallet/deposit — 綠寶石入金(server /pay)回報。body: { mc_uuid?, mc_name, amount, ref }。
// 回 { ok, reason?, discord_id?, balance?, duplicate }。網路/4xx/5xx 由 request 拋 WmsHttpError。
function walletDeposit(cfg, body) { return request(cfg, '/api/v1/wallet/deposit', body, { name: 'WMS-DEPOSIT' }); }
// POST /api/v1/wallet/withdraw — 代玩家出金(錢包提領)。body: { mc_uuid?, mc_name, amount, ref }。
// 回 { ok, reason?, amount, balance, duplicate }。WMS 扣款成功(ok)後,bot 才 /pay 付出綠寶石。
function walletWithdraw(cfg, body) { return request(cfg, '/api/v1/wallet/withdraw', body, { name: 'WMS-WITHDRAW' }); }
