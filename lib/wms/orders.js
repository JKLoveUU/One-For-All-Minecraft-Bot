...
async function executeOrder(wms, bot, order, onProgress) {
    // clean_pa = 清空重製揀貨區:沿用 depositPickingArea(把 accept/reject 箱清空存回倉庫)。
    // depositPickingArea 內部會自行到達 PA(同倉庫 PA 不多餘傳送),不需先白跑一趟倉庫。
    if (order.optype === 'clean_pa') {
        return wms.depositPickingArea(bot, order.picking_area, onProgress);
    }

    // Worker intake(build / clear):單元派工。在 gotoWarehouse 之前處理 —— bot 應前往
    // 建造/清理現場,而非倉庫;材料補給由建造/清理模組內部(materialsMode:'wms')處理。
    // 欄位寬鬆:WMS 可用 optype 或 function 標示。lazy-require worker 避免循環依賴。
    const workerFn = (order.optype === 'build' || order.optype === 'clear') ? order.optype : order.function;
    if (workerFn === 'build' || workerFn === 'clear') {
        const worker = require('./worker');
        return workerFn === 'build'
            ? worker.executeBuild(wms, bot, order, onProgress)
            : worker.executeClear(wms, bot, order, onProgress);
    }
    ...
    switch (order.optype) {
        ...
        case 'update_barrels': {
            const start = order.extra_info?.start ?? 0;
            const end   = order.extra_info?.end   ?? 0;
            return wms.updateBarrels(bot, start, end);
        }
        ...
        for (let i = 0; i < tg.quantity; i++) {
            try {
                ...
                for (const operation of operations) {
                    await wms.executeOpCommit(bot, operation);
                }
                ...
                wms.log(false, 'ERROR', bot.username, `處理拆箱: ${tg.itemName} ${i + 1} / ${tg.quantity} 錯誤: ${err.stack || err.message}`);
            }
        }
    }
    await wms.depositInventory(bot);
    ...
    for (const tg of tgs) {
        for (let i = 0; i < tg.quantity; i++) {
            try {
                ...
                wms.log(false, 'ERROR', bot.username, `處理裝箱: ${tg.itemName} ${i + 1} / ${tg.quantity} 錯誤: ${err.stack || err.message}`);
            }
        }
    }
    await wms.depositInventory(bot);
...
async function buyAtShop(wms, bot, order) {
    ...
    await pathfinder.astarfly(bot, standPos.offset(0, 1, 0), null, null, null, true);
    for (let t = 0; t < 3; t++) {
        await pathfinder.astarfly(bot, standPos, null, null, null, true);
        await sleep(150);
    }
...
async function orderTransfer(wms, bot, order, onProgress) {
    ...
    // 缺貨原因:來源 buy 沒貨 → 'source_empty';目的 sell 收不下/店主沒錢 → 'dest_full'。
    // 設定後在結束時以 wms.reportFail 回報,觸發後端「冷卻該牌 + 訂單標記完成」(不重派)。
    let depletionReason = null;

    // WMS progress 回報(顯示用):placed = 已完成的買賣半步、total = 目標次數
    // (count=-1 無限 → total=0,WMS 不顯示進度條只當 keepalive)。emit 觸發很頻繁,
    // 故節流到最多每 WMS_REPORT_THROTTLE_MS 回報一次,避免洗 WMS / Discord。best-effort。
    const WMS_REPORT_THROTTLE_MS = 5000;
    let lastWmsReport = 0;
    function reportWmsProgress(force = false) {
        const now = Date.now();
        if (!force && now - lastWmsReport < WMS_REPORT_THROTTLE_MS) return;
        lastWmsReport = now;
        const total = (count > 0) ? count : 0;
        // 有指定數量 → placed=已完成半步數;無限量(count=-1)→ 用已實際送達目的的數量(sellQty)
        // 當作進度,total=0 不顯示百分比,但訂單頁可顯示「已搬運 N」。
        const done = (total > 0)
            ? Math.min(total, Math.max(0, total - Math.max(0, remaining)))
            : sellQty;
    ...
    let buytype;
    {
        ...
        if (buyBlock.name.includes('chest') || buyBlock.name.includes('barrel')) {
            ...
            if (shopInfo.count === 0) {
                // 來源一開始就沒貨:回報缺貨(後端冷卻該牌 + 標記完成),回倉清空後結束。
                log(true, 'INFO', `購買商店初始庫存為0,回報 source_empty 並結束`);
                await wms.reportFail(order.order_id, 'source_empty',
                    { transfer_moved: 0, transfer_cost: 0, transfer_income: 0 }).catch(() => { });
                await sleep(1500);
                await wms.gotoWarehouse(bot);
                await wms.depositInventory(bot);
                return { reported: true, reason: 'source_empty' };
            }
        }
    }
    ...
    for (let i = 0; i < 99999; i++) {
        ...
        if (sellShopNoFund) { log(true, 'INFO', `店主 ${sellWarp} 收購金額不足`); depletionReason = 'dest_full'; break; }
        ...
        if (crtModeBit === 0) {
            ...
            for (let r = 0; r < 3; r++) {
                ...
                if (dist > 4) {
                    ...
                    if(r>2) log(false, 'WARN', `購買 touchChestShop 失敗，距離近只重新尋路 retry=${r} dist=${dist.toFixed(1)}`);
                    for (let j = 0; j < 3; j++) { await pathfinder.astarfly(bot, buyStandPos, true); await sleep(500); }
                }
                // await sleep(5000)
                info = await mcFallout.touchChestShop(bot, buySignPos,1000+r*500);
                if (info?.owner != null) break;
                if(r>2) log(false, 'WARN', `購買 touchChestShop 失敗 retry=${r}`);
                info = null; await sleep(50);
            }
            ...
            if (info.commodity_count > 0) {
                ...
                if (noitem) { log(true, 'INFO', `商店售出庫存為0 停止`); shouldEnd = true; depletionReason = 'source_empty'; }
            ...
            for (let r = 0; r < 3; r++) {
                ...
                if (dist > 4) {
                    ...
                    if(r>2) log(false, 'WARN', `出售 touchChestShop 失敗，距離近只重新尋路 retry=${r} dist=${dist.toFixed(1)}`);
                    for (let j = 0; j < 3; j++) { await pathfinder.astarfly(bot, sellStandPos, true); await sleep(500); }
                }
                // await sleep(5000)
                info = await mcFallout.touchChestShop(bot, sellSignPos,1000+r*500);
                if (info?.owner != null) break;
                if(r>2) log(false, 'WARN', `出售 touchChestShop 失敗 retry=${r}`);
                info = null; await sleep(50);
            }
            ...
            if (info.space > 3) {
                bot.chat('/qs amount all');
                await sleep(4000);
            } else if (info.space > 0 ) { //3 - 0 效率低下模式
                if (noslot) { log(true, 'INFO', `商店收購 效率低下模式 或接近滿 停止`); shouldEnd = true; depletionReason = 'dest_full'; }
                else { noslot = true; await sleep(5000); }
                continue;
            } else if (info.space === 0) {
                if (noslot) { log(true, 'INFO', `商店收購空間為 0 停止`); shouldEnd = true; depletionReason = 'dest_full'; }
                else { noslot = true; await sleep(5000); }
                continue;
            }
    ...
    // 結束統計 + 缺貨回報:
    //  1) 先強制回報一次最終進度(無限量訂單可顯示「已搬運 N」)。
    //  2) 若是缺貨結束 → reportFail(source_empty/dest_full):後端冷卻該牌 + 訂單標記完成;
    //     此時回傳 reported:true,呼叫端(warehouse.js)即不再送 finish,避免重覆回報。
    //  3) 一般結束 → reported:false,呼叫端照常送 finish。
    reportWmsProgress(true);
    if (depletionReason) {
        log(true, 'INFO', `回報缺貨 ${depletionReason}(冷卻該牌 + 標記完成)`);
        await wms.reportFail(order.order_id, depletionReason,
            { transfer_moved: sellQty, transfer_cost: totalCost, transfer_income: totalIncome }).catch(() => { });
    }
    return {
        reported: !!depletionReason,
        reason: depletionReason,
        stats: { buyTrips, sellTrips, buyQty, sellQty, totalCost, totalIncome, profit },
    };
}
