...
const wms = {
    ...
    async depositInventory(bot) {
        ...
        for (let i = 0; i <= 44; i++) {
            ...
            const results = await this.getDeposit(this.cfg, {
                id: p.item, quantity: 1, barreltype: 'full_shulker', requiretype: 'deposit',
            });
            for (const result of results) {
                const ok = await this.executeOperation(bot, result);
                if (ok) await this.commit(this.cfg, result.id);
    ...
    // ── Picking area ops ────────────────────────────────────────────────────
    depositPickingArea(bot, id)          { return picking.depositPickingArea(this, bot, id); },
