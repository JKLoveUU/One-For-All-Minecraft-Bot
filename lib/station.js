...
const station = {
    ...
    oldrestock: async function (bot, stationConfig, RS_obj_array) {
        ...
        async function st_restock_single(stationConfig, restockid, quantity) {
            ...
            if(findItemMaterialsIndex == -1){
                console.log(`材料站設定找不到材料 ${restockid}`);
            }
            await mcFallout.promiseTeleportServer(bot, stationConfig.stationServer, 15_000)
            ...
            if (quantity == -1) {
                ...
                let shu;
                for (let t = 0; t < 5; t++) {
                    if (!bot.blockAt(shulkerBox_loc)) { await sleep(1000); continue }
                    if (bot.blockAt(shulkerBox_loc)?.name == 'air') {
                        await bot.activateBlock(bot.blockAt(botton_loc));
                        await sleep(300);
                    }
                    shu = await containerOperation.openContainerWithTimeout(bot, shulkerBox_loc, 1500)
                    if (shu) { console.log("開盒子成功"); break; }
                    console.log("開盒子失敗"); await sleep(200);
                }
                if (shu) {
                ...
                if (remain > 0 || remain == -1) {
                    ...
                    let ofShu;
                    for (let t = 0; t < 5; t++) {
                        if (!bot.blockAt(overfull_shu_loc)) { await pathfinder.astarfly(bot, overfull_standPos, null, null, null, true); await sleep(50); continue }
                        ofShu = await containerOperation.openContainerWithTimeout(bot, overfull_shu_loc, 1500)
                        if (ofShu) { console.log("開盒子成功"); break; }
                        console.log("開盒子失敗"); await sleep(200);
                    }
                    if (!ofShu) return;
                    remain = await containerOperation.deposit(bot, ofShu, restockid, -1, false);
                    ofShu.close();
                }
            } else {
                // 取出
                ii: for (let maxTryTime = 0; maxTryTime < 10; maxTryTime++) {
                    if (remain <= 0) break;
                    if (bot.blockAt(shulkerBox_loc)?.name == 'air') {
                        await bot.activateBlock(bot.blockAt(botton_loc));
                        await sleep(300);
                    ...
                    const shu = await containerOperation.openContainerWithTimeout(bot, shulkerBox_loc, 1500)
                    if (!shu) { console.log("開盒子失敗"); await sleep(1000); continue ii; }
                    console.log("提取中...")
                    const tempremain = await containerOperation.withdraw(bot, shu, restockid, remain, false);
                    shu.close();
                    if (tempremain == -2) {
                        console.log("盒子空了 點及按鈕")
                        await bot.activateBlock(bot.blockAt(botton_loc));
